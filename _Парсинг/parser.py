...
users_agent = ('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.0.0.0 Safari/537.36',
...
res_main = requests.get('https://www.dotabuff.com/matches/' + code, headers=headers)
time.sleep(1)
res_kills = requests.get('https://www.dotabuff.com/matches/' + code + '/kills', headers=headers)
time.sleep(1)
res_obj = requests.get('https://www.dotabuff.com/matches/' + code + '/objectives', headers=headers)

soup_main = BeautifulSoup(res_main.text, 'lxml')
soup_obj = BeautifulSoup(res_obj.text, 'lxml')
soup_kills = BeautifulSoup(res_kills.text, 'lxml')
# SITE ---------------------------------------------------------------------------------------------------------------------------------------
